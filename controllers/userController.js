...
module.exports.chatRoom = async (req, res) => {
  ...
  try {
    ...
    if(!res.locals.currentRoom.users.find(elem => elem.email === req.session.email)) //check is user in chat ?
        return res.redirect('/user/404');
...
module.exports.createGroupPOST = async (req,res) => {
  ...
  try{
    ...
    for(const user of users)
    {
      const isUpdated = await Users.updateOne({email : user},{$push : { rooms: { id: room.id } }});
      if(!isUpdated)
        throw new Error();
    }
